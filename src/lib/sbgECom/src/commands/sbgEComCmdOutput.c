		...
		for (trial = 0; trial < pHandle->numTrials; trial++)
		{
			...
			if (errorCode == SBG_NO_ERROR)
			{
				...
				if (errorCode == SBG_NO_ERROR)
				{
					...
					//
					outputPort = (SbgEComOutputPort) (castu8 = sbgStreamBufferReadUint8LE(&inputStream));
					msgId = sbgStreamBufferReadUint8LE(&inputStream);
					classId = (SbgEComClass) (castu8 = sbgStreamBufferReadUint8LE(&inputStream));
					*pConf = (SbgEComOutputMode) (castu16 = sbgStreamBufferReadUint16LE(&inputStream));
		...
		for (trial = 0; trial < pHandle->numTrials; trial++)
		{
			...
			if (errorCode == SBG_NO_ERROR)
			{
				...
				if (errorCode == SBG_NO_ERROR)
				{
					...
					// First is returned outputPort, then messageId, classId rate and the output configuration at last.
					//
					outputPort = (SbgEComOutputPort) (cast = sbgStreamBufferReadUint8LE(&inputStream));
					classId = (SbgEComClass) (cast = sbgStreamBufferReadUint8LE(&inputStream));
					*pEnable = (bool) (cast = sbgStreamBufferReadUint8LE(&inputStream));
	...
	for (trial = 0; trial < pHandle->numTrials; trial++)
	{
		...
		if (errorCode == SBG_NO_ERROR)
		{
			...
			if (errorCode == SBG_NO_ERROR)
			{
				...
				// First is returned outputPort, then messageId, classId rate and the output configuration at last.
				//
				internalId = (SbgECanMessageId) (castu16 = sbgStreamBufferReadUint16LE(&inputStream));
				*pMode = (SbgEComOutputMode) (castu16 = sbgStreamBufferReadUint16LE(&inputStream));
				*pUserId = sbgStreamBufferReadUint32LE(&inputStream);
				*pExtended = (bool) (castu8 = sbgStreamBufferReadUint8LE(&inputStream));
	...
	for (trial = 0; trial < pHandle->numTrials; trial++)
	{
		...
		if (errorCode == SBG_NO_ERROR)
		{
			...
			if (errorCode == SBG_NO_ERROR)
			{
				...
				pConf->format = (SbgEComLegacyFormat) (castu8 = sbgStreamBufferReadUint8LE(&inputStream));
				pConf->endian = (SbgEComLegacyEndian) (castu8 = sbgStreamBufferReadUint8LE(&inputStream));
				pConf->mode = (SbgEComOutputMode) (castu16 = sbgStreamBufferReadUint16LE(&inputStream));
